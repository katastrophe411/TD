...
#include UE_INLINE_GENERATED_CPP_BY_NAME(DynamicListViewBase)

#define LOCTEXT_NAMESPACE "UMG"

UDynamicListViewBase::UDynamicListViewBase(const FObjectInitializer& ObjectInitializer)
...
#if WITH_EDITOR
const FText UDynamicListViewBase::GetPaletteCategory()
{
	return LOCTEXT("Lists", "Lists");
}

void UDynamicListViewBase::ValidateCompiledDefaults(IWidgetCompilerLog& CompileLog) const
{
	if (!EntryWidgetClass)
	{
...
void UDynamicListViewBase::RegenerateAllEntries()
{
...
void UDynamicListViewBase::ScrollToTop()
{
	if (MyTableViewBase.IsValid())
	{
...
void UDynamicListViewBase::ScrollToBottom()
{
	if (MyTableViewBase.IsValid())
	{
...
void UDynamicListViewBase::SetScrollOffset(const float InScrollOffset)
{
	if (MyTableViewBase.IsValid())
	{
...
void UDynamicListViewBase::SetWheelScrollMultiplier(float NewWheelScrollMultiplier)
{
...
void UDynamicListViewBase::SetScrollbarVisibility(ESlateVisibility InVisibility)
{
	if (MyTableViewBase)
	{
...
const TArray<UUserWidget*>& UDynamicListViewBase::GetDisplayedEntryWidgets() const
{ 
	return EntryWidgetPool.GetActiveWidgets(); 
}

float UDynamicListViewBase::GetScrollOffset() const
{
	if (MyTableViewBase.IsValid())
	{
...
TSharedRef<SWidget> UDynamicListViewBase::RebuildWidget()
{
	FText ErrorText;
	if (!EntryWidgetClass)
	{
...
void UDynamicListViewBase::ReleaseSlateResources(bool bReleaseChildren)
{
...
void UDynamicListViewBase::SynchronizeProperties()
{
...
TSharedRef<SDynamicTableViewBase> UDynamicListViewBase::RebuildListWidget()
{
	...
	return SNew(SDynamicListView<TSharedPtr<FString>>);
}

void UDynamicListViewBase::RequestRefresh()
{
	if (MyTableViewBase.IsValid())
	{
...
void UDynamicListViewBase::HandleRowReleased(const TSharedRef<ITableRow>& Row)
{
...
void UDynamicListViewBase::FinishGeneratingEntry(UUserWidget& GeneratedEntry)
{
	if (!IsDesignTime())
	{
		...
		if (!EntryGenAnnouncementTimerHandle.IsValid())
		{
			...
			{
				EntryGenAnnouncementTimerHandle = World->GetTimerManager().SetTimerForNextTick(this, &UDynamicListViewBase::HandleAnnounceGeneratedEntries);
			}
		}
	}
}

void UDynamicListViewBase::HandleAnnounceGeneratedEntries()
{
