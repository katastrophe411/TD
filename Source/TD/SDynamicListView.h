 ...
 */
	...
	TSharedRef<class ITableRow>,
		...
		const TSharedRef< class SDynamicTableViewBase >& );
};
...
class SDynamicListView : public SDynamicTableViewBase, TListTypeTraits<ItemType>::SerializerType, public ITypedTableView<ItemType>
{
public:
	...
	using FOnGenerateRow            = typename TDynamicListDelegates< ItemType >::FOnGenerateRow; // Fix
...
public:
	SLATE_BEGIN_ARGS(SDynamicListView<ItemType>)
	...
	void Construct(const typename SDynamicListView<ItemType>::FArguments& InArgs)
	{
		...
		this->OnItemToString_Debug = InArgs._OnItemToString_Debug.IsBound()
			...
			: SDynamicListView< ItemType >::GetDefaultDebugDelegate();
	...
	PRAGMA_DISABLE_DEPRECATION_WARNINGS
	SDynamicListView(ETableViewMode::Type InListMode = ETableViewMode::List)
		: SDynamicTableViewBase(InListMode)
...
public:
	...
	virtual FReply OnKeyDown( const FGeometry& MyGeometry, const FKeyEvent& InKeyEvent ) override
	{
		...
		return SDynamicTableViewBase::OnKeyDown(MyGeometry, InKeyEvent);
	}
...
public:
	...
	virtual FNavigationReply OnNavigation(const FGeometry& MyGeometry, const FNavigationEvent& InNavigationEvent) override
	{
		...
		return SDynamicTableViewBase::OnNavigation(MyGeometry, InNavigationEvent);
	}
	...
	virtual FReply OnMouseButtonDown( const FGeometry& MyGeometry, const FPointerEvent& MouseEvent ) override
	{
		...
		return SDynamicTableViewBase::OnMouseButtonDown(MyGeometry, MouseEvent);
	}
	...
	virtual FReply OnMouseButtonUp( const FGeometry& MyGeometry, const FPointerEvent& MouseEvent ) override
	{
		...
		return SDynamicTableViewBase::OnMouseButtonUp(MyGeometry, MouseEvent);
	}
	...
	/**
	* An accessible implementation for SDynamicListView to be exposed to platform accessibility APIs.
	* For subclasses of SDynamicListView, inherit from this class and override and functions to 
	...
	*/
	...
	public:
		...
		{ 
			...
			if (Widget.IsValid())
			{
				TSharedPtr<SDynamicListView<ItemType>> ListView = StaticCastSharedPtr<SDynamicListView<ItemType>>(Widget.Pin());
	...
	virtual TSharedRef<FSlateAccessibleWidget> CreateAccessibleWidget() override
	{
		...
		return MakeShareable<FSlateAccessibleWidget>(new SDynamicListView<ItemType>::FSlateAccessibleListView(SharedThis(this), WidgetType));
	}
...
private:
	...
	{
	public:
		FWidgetGenerator(SDynamicListView<ItemType>* InOwnerList)
			: OwnerList(InOwnerList)
			, TotalItemsLastGeneration(0)
		{
		}
		...
		SDynamicListView<ItemType>* OwnerList;
...
public:	
	...
	virtual FReGenerateResults ReGenerateItems( const FGeometry& MyGeometry ) override
	{
		...
		if (Items.Num() > 0)
		{
			...
			int32 StartIndex = 0;
			double PrevItemsLength = 0;
			for ( int32 Index = 0; Index < CachedItemLengths.Num(); Index++ )
			{
				PrevItemsLength += CachedItemLengths[Index];

				if (CurrentScrollOffset < PrevItemsLength)
				{
					StartIndex = Index;
					break;
				}
			...
			for( int32 ItemIndex = StartIndex; !bHasFilledAvailableArea && ItemIndex < Items.Num(); ++ItemIndex )
			{
				...
				if (bIsFirstItem)
				{
					...
					float FirstItemFraction = 1.f;
					for (const float CachedItemLength : CachedItemLengths)
					{
						SumOfItemLengths += CachedItemLength;
			
						if (CurrentScrollOffset <= SumOfItemLengths)
						{
							FirstItemFraction = (SumOfItemLengths - CurrentScrollOffset) / CachedItemLength;
							break;
						}
					}
					...
					// ItemsInView += FirstItemVisibleFraction; // TODO
					ItemsInView += FirstItemFraction;
				}
			...
			if (bAtEndOfList && !bHasFilledAvailableArea)
			{
				...
				float NewOffset = GetTotalItemsLength() - MyGeometry.GetLocalSize().Y;
				return FReGenerateResults(NewOffset, LengthGeneratedSoFar, ItemsInView, true);
			}
	...
	void GeneratePinnedWidgetForItem(const ItemType& CurItem, int32 ItemIndex, int32 NumPinnedItems, float LayoutScaleMultiplier)
	{
		...
		// We wrap the row widget around an SDynamicListViewPinnedRowWidget for custom styling
	...
	virtual TSharedRef<ITableRow> GenerateNewPinnedWidget(ItemType InItem, const int32 ItemIndex, const int32 NumPinnedItems)
	{
		...
		else
		{
			...
			TSharedRef< SDynamicTableRow<ItemType> > NewListItemWidget =
				SNew(SDynamicTableRow<ItemType>, SharedThis(this))
				.Content()
				[
					SNew(STextBlock).Text(NSLOCTEXT("SDynamicListView", "OnGeneratePinnedRowNotAssignedMessage", "OnGeneratePinnedRow() not assigned."))
				];
	...
	virtual TSharedRef<ITableRow> GenerateNewWidget(ItemType InItem)
	{
		...
		else
		{
			...
			TSharedRef< SDynamicTableRow<ItemType> > NewListItemWidget = 
				...
				.Content()
				[
					SNew(STextBlock) .Text( NSLOCTEXT("SDynamicListView", "OnGenerateWidgetNotAssignedMessage", "OnGenerateWidget() not assigned.") )
				];
...
protected:
	...
	virtual void PopulateTotalItemsLength(float LayoutScaleMultiplier) override
	{
		...
		TSharedPtr<SObjectDynamicTableRow<ItemType>> RowWidget = StaticCastSharedPtr<SObjectDynamicTableRow<ItemType>>(GenerateNewWidget(nullptr).ToSharedPtr());
		if (!RowWidget.IsValid())
		{
			return;
		}
		...
		for (ItemType CurItem : GetItems())
		{
			...
			const TSharedRef<SWidget> NewlyGeneratedWidget = RowWidget->AsWidget();
			NewlyGeneratedWidget->MarkPrepassAsDirty();
			NewlyGeneratedWidget->SlatePrepass(LayoutScaleMultiplier);
	
			const bool bIsVisible = NewlyGeneratedWidget->GetVisibility().IsVisible();
			FTableViewDimensions GeneratedWidgetDimensions(Orientation, bIsVisible ? NewlyGeneratedWidget->GetDesiredSize() : FVector2D::ZeroVector);
	
			TotalItemsLength += GeneratedWidgetDimensions.ScrollAxis;
			CachedItemLengths.Add(GeneratedWidgetDimensions.ScrollAxis);
		}
	}

	virtual double GetTotalItemsLength() const override
	{
...
protected:
	...
	/** Container for, Item to Widget length cache */
	TArray<float> CachedItemLengths;
	
	/** The length of all item */
	double TotalItemsLength;
