...
template <typename ItemType> class SDynamicListView;

template<typename ItemType>
class SDynamicTableRow : public ITableRow, public SBorder
{
...
public:
	...
	SLATE_BEGIN_ARGS( SDynamicTableRow< ItemType > )
		...
		SLATE_DEFAULT_SLOT( typename SDynamicTableRow<ItemType>::FArguments, Content )
	 ...
	 */
	void Construct(const typename SDynamicTableRow<ItemType>::FArguments& InArgs, const TSharedRef<SDynamicTableViewBase>& InOwnerTableView)
	{
		/** Note: Please initialize any state in ConstructInternal, not here. This is because SDynamicTableRow derivatives call ConstructInternal directly to avoid constructing children. **/
	...
	/**
	* An accessible implementation of SDynamicTableRow exposed to platform accessibility APIs.
	* For subclasses of SDynamicTableRow, inherit from this class and override any functions
	...
	*/
	...
	public:
		...
		virtual void Select() override
		{
			if (Widget.IsValid())
			{
				TSharedPtr<SDynamicTableRow<ItemType>> TableRow = StaticCastSharedPtr<SDynamicTableRow<ItemType>>(Widget.Pin());
	...
	virtual TSharedRef<FSlateAccessibleWidget> CreateAccessibleWidget() override
	{
		...
		return MakeShareable<FSlateAccessibleWidget>(new SDynamicTableRow<ItemType>::FSlateAccessibleTableRow(SharedThis(this), WidgetType));
	}
#endif
	...
	virtual FReply OnMouseButtonUp( const FGeometry& MyGeometry, const FPointerEvent& MouseEvent ) override
	{
		...
		// Requires #include "Widgets/Views/SListView.h" in your header (not done in SDynamicTableRow.h to avoid circular reference).
...
protected:
	 ...
	 */
	void ConstructInternal(FArguments const& InArgs, TSharedRef<SDynamicTableViewBase> const& InOwnerTableView)
	{
		bProcessingSelectionTouch = false;
		...
		SetBorderImage(TAttribute<const FSlateBrush*>(this, &SDynamicTableRow::GetBorder));

		this->SetForegroundColor(TAttribute<FSlateColor>( this, &SDynamicTableRow::GetForegroundBasedOnSelection ));
	...
	void SetOwnerTableView( TSharedPtr<SDynamicTableViewBase> OwnerTableView )
	{
		...
		// See SListView.
		this->OwnerTablePtr = StaticCastSharedPtr< SDynamicListView<ItemType> >(OwnerTableView);
	}
...
protected:
	...
	/** @see SDynamicTableRow's OnCanAcceptDrop event */
	FOnCanAcceptDrop OnCanAcceptDrop;

	/** @see SDynamicTableRow's OnAcceptDrop event */
...
template<typename ItemType>
class SMultiColumnDynamicTableRow : public SDynamicTableRow<ItemType>
{
public:
	...
	/** Use this to construct the superclass; e.g. FSuperRowType::Construct( FTableRowArgs(), OwnerTableView ) */
	typedef typename SDynamicTableRow<ItemType>::FArguments FTableRowArgs;

protected:
	void Construct(const FTableRowArgs& InArgs, const TSharedRef<SDynamicTableViewBase>& OwnerTableView)
	{
		SDynamicTableRow<ItemType>::Construct(
			FTableRowArgs()
			.Style(InArgs._Style)
			.ExpanderStyleSet(InArgs._ExpanderStyleSet)
	...
	{
		SDynamicTableRow<ItemType>::Content = InContent;
